function [m,v,w,r]=asym_gaussmix_his_vals_limits(f,s,k,limits,m_init,v_init,w_init,r_init,init_type,l,graph,fft_m)
...
if isempty(limits) | strcmp(init_type,'0') 
    m=zeros(1,k);
    v=zeros(1,k);
    w=zeros(1,k);
    r=ones(1,k);
...
elseif ~isempty(limits)
    m=((limits(:,1) + limits(:,2))/2 )';
    v=(((limits(:,2) - limits(:,1))/2).^2)';
    w=w_init';
    r=r_init';
else
    m=m_init;
...
while (niter<=maxiter) %& (abs(100*(Ln-Lo)/Lo)>th)
    ...
    m_old=m;
    r_old=r;
    [m,v,w,r] = Maximization(f,limits,k,pg,pyp,m_old,r_old);  % M-step
...
function [m,v,w,r] = Maximization(f,limits,k,pg,pyp,m_old,r_old) %removing m
[d,N]=size(f);
m_temp=zeros(1,k);
v_temp=zeros(1,k);
w_temp=zeros(1,k);
