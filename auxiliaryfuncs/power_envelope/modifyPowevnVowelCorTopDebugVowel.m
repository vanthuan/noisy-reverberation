function [linearmodifiedPowerenv16kHzs,envsyndur, vowdur_all, condur_all,linearmodifiedPowerenv] = modifyPowevnVowelCorTopDebugVowel(mapId,limitBound,n_all,typeEnvelope,date,jj,m,mx,x_syndurF0,vowels,powerRatiosNeutral,PowEnvNeutral1KHz,f0rawSynLombard,fs,shiftedEnv,phrase_info_numNeutral, phrase_info_num,phrase_info,noiseLevels, workfolderpowerenvelope,gender,word,fig)

% x_syndurF0_new = generateFixNoisySpeech(x_syndurF0,noise,fs);
[envsyndur]=PEdetection(x_syndurF0,1,fs)';
m = fix(min(phrase_info_num(2,:))/2)
mx=5;
...
[bq,xlmq,cq] = spect08v23(m,mx,logPowerF0, phrase_info_num);
...
%% recorrect annotation
groups
p_lambda_groups= [0];
for ii=1:length(groups)-1
    if groups(ii) < groups(ii+1)
        p_lambda_groups =[p_lambda_groups p_lambda(ii+1)];
    end
end
p_lambda_groups =sort(unique([p_lambda_groups p_lambda(end)]));
annotations = fix([phrase_info_num(1,1:end) phrase_info_num(1,end)+phrase_info_num(2,end)]);
annotations_new = [p_lambda_groups(1)];
for nn=2:length(annotations)-1
    [dists,ids]= sort(abs(annotations(nn) - p_lambda_groups));
    kk=1;
    while kk <= length(ids) && p_lambda_groups(ids(kk)) <= annotations_new(end)
        kk = kk +1;
    end
    if  kk <= length(ids)
        annotations_new =[annotations_new p_lambda_groups(ids(kk)) ];
    else
        annotations_new =[annotations_new annotations(nn)];
    end
end

% annotations_new =sort(unique([annotations_new p_lambda_groups(end) ]))
% if length(annotations_new) == length(phrase_info_num(1,:))+1
%     
%     phrase_info_num(1,:) = annotations_new(1:end-1);
%     phrase_info_num(2,:) = annotations_new(2:end) - annotations_new(1:end-1);
% end
...
for kk=1:length(phrase_info_num(1,:)),
    ...
    if phrase_info_num(4,kk) == 1 || (phrase_info_num(4,kk) == 0 && length(find(phrase_info_num(3,:) == phrase_info_num(3,kk))) == 1)
        ...
        for ii=1:length(n_i_all)
            if mark(ii) == 0
                A = n_i_all{ii};
                ii_gr =find(groups == groups(ii));

                 ii_gr = setdiff(ii_gr,ii);
                if ~isempty(ii_gr)
                    for tt=1:length(ii_gr)
                        A = [A n_i_all{ii_gr(tt)}];
                    end
                    
                end
                A = sort(unique(A));
                if (B(1) <= A(1) && B(end) >= A(end))
                    ...
                    if length(C_cur) > 1
                        ...
                        if ~isempty(ii_gr)
                            ...
                            for tt=1:length(ii_gr)
                                A = n_i_all{ii_gr(tt)};
                                C_cur2= [C_cur2 intersect(A,B)];
                            end
                            if length(C_cur) > length(C_cur2)
                                b_ratios_vowel(find(groups == groups(ii))) = kk;
                                mark(find(groups == groups(ii))) = 1;
                            end
                        else
    ...
    if phrase_info_num(4,kk) == 0
        ...
        for ii=1:length(n_i_all)
            if mark(ii) == 0
                ...
                A = sort(unique(A));
                if (B(1) <= A(1) && B(end) >= A(end))|| (A(1) <= B(1) && A(end) >= B(end))
...
if fig == 1
    ...
    grid on;
    title(['vowels: ' '/' word '/ (A ' gender ' speaker) ' noiseLevelStr ]);
...
for mm=1:maxMora
    ...
    if ~isempty(vowels_indices)
        if mm > 1 && length(vowels_indices) == 1 && (strcmp(phrase_info(1,vowels_indices),'n') == 1 ||  (strcmp(phrase_info(1,vowels_indices),'u') == 1 && phrase_info_num(4,vowels_indices-1) == 1 ))
...
%         brqup =[ ones(1,length(brq0(brRatios_unique > 0)))*max([logPowerF0; lnrmsPowerMax])./b_groups_min_trace(brRatios_unique(brRatios_unique > 0))];
    ...
    brqup =  ones(1,length(brq0(brRatios_unique > 0)))*limitBound./b_groups_min_trace(brRatios_unique(brRatios_unique > 0));
    ...
    for mm=1:length(durMoras)
        if ~isempty(durMoras{mm})
            powerMoras(mm) = 10*log10(rms((PowEnvNeutral1KHz(unique(fix(mapId(durMoras{mm}(durMoras{mm}<=length(PowEnvNeutral1KHz)))))))));
        end
