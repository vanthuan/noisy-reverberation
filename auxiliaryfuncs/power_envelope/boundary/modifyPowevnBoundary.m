function [linearmodifiedPowerenv16kHzs,envsyndur, vowdur_all, linearmodifiedPowerenv] = modifyPowevnBoundary(mapId,limitBound,n_all,typeEnvelope,date,jj,m,mx,x_syndurF0,vowels,powerRatiosNeutral,PowEnvNeutral1KHz,f0rawSynLombard,fs,shiftedEnv,phrase_info_numNeutral, phrase_info_num,phrase_info,noiseLevels, workfolderpowerenvelope,gender,word,fig)
...
for kk =1:length(phrase_info_num(1,:))
    ...
    if ~isempty (y_changed)
        ...
        for ii=1:length(y_changed) -1
            n_i = n_p(y_changed(ii):y_changed(ii+1));
            if length(n_i) < 5
...
for mm=1:maxMora,
    ...
    if ~isempty(mInds)
        ...
        for vv=1:length(mInds),
            ...
            if ~isempty(find(strcmp(phrase_info(1,mInds(vv)),vowels) == 1, 1)) || (strcmp(phrase_info(1,mInds(vv)),'n') == 1 && length(mInds) == 1)  || length(mInds) == 1
                vow_dur = [vow_dur dur];
            ...
            if mm > 1 && length(mInds) == 1 && strcmp(phrase_info(1,mInds(vv)),'n') == 1 
                b_ratios_vowel_mora(mInds(vv)) = mm-1;
                durMoras{mm-1} = sort(unique([durMoras{mm-1} dur]));

            elseif phrase_info_num(4,mInds(vv)) ~= -1
                b_ratios_vowel_mora(mInds(vv)) = mm;
                durMoras{mm} = sort(unique([durMoras{mm} dur]));
            end
        end
...
if length(durMoras) > 2
    expectedPowerMoraRatio = powerMoraRatios' + [ratio21(1)  repmat(ratio31(1),1,length(durMoras)-3) ratio41(1)];
else
...
expectedPowerMoraRatioStat = [ratio21(2)  repmat(ratio31(2),1,length(durMoras)-3) ratio41(2)];
...
if length(durMoras) > 1
    ...
    brqup =  10./b_max;
     ...
     nonlinear = @(brq)moraCeqBoundary(brq,b_ratios_refined,phoneme_sets,phrase_info_num,durMoras,maxMora,vowdurs,condurs,shiftedEnv,expectedPowerMoraRatioStat,expectedPowerMoraRatioMin,expectedPowerMoraRatioMax,expectedVCRatio,type,N);
