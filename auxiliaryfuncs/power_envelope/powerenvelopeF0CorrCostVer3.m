function absdiffCorr = powerenvelopeF0CorrCostVer3(brq,b_ratios,bq,N,lambdas,negative_lambdas_pos,aq,cq,n_is,is,n1s,F0,postivePoints,OptCorr,expectedPower,condurs,vowdurs,expectedRatio,shiftedEnv,expectedMoraRatio,durMoras)
...
cal_corrF0 =corr(powerenv(postivePoints)',log(F0(postivePoints))');
...
% power information
 ...
 absdiffCorr = (abs(cal_corrF0-OptCorr) +  abs(log(rms(exp(powerenv))) - expectedPower) + sum(abs(powerMoraRatios - expectedMoraRatio))+ sum(abs(powerRatios - expectedRatio)))/9 ;