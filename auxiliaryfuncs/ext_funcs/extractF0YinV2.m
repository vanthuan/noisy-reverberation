function [f0raw_yin,noMora,h_fig] = extractF0YinV2(x_db,p, f0raw_db,phrase_info_num,phrase_info,voiced_consonants,word,ndBLabel,gender,accurateestimate, fig)
h_fig = 0;
p.bufsize = 5000;
r=yin(x_db,p);
...
for kk=1:length(phrase_info_num(1,:)),
    if phrase_info_num(4,kk) ==1
        ...
        if kk==1,
            ...
            if ~isempty(index)
                f0raw_yin_part2 = f0raw_yin(fix(phrase_info_num(1,kk) + phrase_info_num(2,kk))+1: fix(phrase_info_num(1,index(1)) + phrase_info_num(2,index(1))));
