function [SYN_SE_MIMICS] = GMM_ENVSPS_TOP_GAUSS_BOTTOM(PHI, A, pos,PL,gauss_addvvl, numsSmooth,changed,gmm_param_up,increase_para_up,gmm_param_up_bottom,increase_para_up_bottom,gmm_para_up_mora,increase_para_up_mora, P,GMM_P, m_order,phrase_info,phrase_info_num,f0rawNeutralDur,f0raw,gender,freq_shifts,amp_scales, vowels, voiced_consonants)
...
F = F*-1;
F = F + maxF + minF;
[GMMSE,GMM_M,GMM_V,GMM_W, OPT_PARA,GMM_MAX] = GMM_Ext_func(F,f0rawNeutralDur(pos),GMM_P); % Spectral GMM
...
mod_spec_gmm_bottom = zeros(513,size(GMM_M,2));
...
ratios =ones(size(gmm_param_up_bottom{1},1),1);
[gauss_add_vowel] = getBigGaussianEnvIncreaseAll(gmm_param_up_bottom{1},ratios,increase_para_up_bottom{1});
ratios =ones(size(gmm_param_up_bottom{2},1),1);
[gauss_add_voicedCon] = getBigGaussianEnvIncreaseAll(gmm_param_up_bottom{2},ratios,increase_para_up_bottom{2});
ratios =ones(size(gmm_param_up_bottom{3},1),1);
[gauss_add_unvoicedCon] = getBigGaussianEnvIncreaseAll(gmm_param_up_bottom{3},ratios,increase_para_up_bottom{3});
gauss_adds_up_bottom = {gauss_add_vowel,gauss_add_voicedCon,gauss_add_unvoicedCon};
...
for ii= 1:size(GMM_M,2);
    ...
    if changed(pos(ii)) == 1
        ...
        target_envelope_up = target_envelope_up.*gauss_adds_up_bottom{tt};
        ...
        hold on;
        plot(((0:512)'/1024*fs),db((spec_tilt_add1- maxF - minF)* -1),'linewidth',1.3);
%         
...
%         close
    ...
    mod_spec_gmm_bottom(:,ii) = gmm_spectraEnv;
    % % %     mod_spec_gmmReplaced(:,ii) = gmm_spectraReplaced;
end

mod_spec_gmm_bottom = mod_spec_gmm_bottom - maxF -minF;
mod_spec_gmm_bottom = mod_spec_gmm_bottom
[~,GMM_M,GMM_V,GMM_W, OPT_PARA,GMM_MAX] = GMM_Ext_func(mod_spec_gmm_bottom,f0raw(pos),fix(GMM_P)); % Spectral GMM
...
[GMM_SE2] = GMM_Ext_func(mod_spec_gmm_bottom,f0raw(pos),fix(GMM_P)); % Spectral GMM
